...
export function jitteredCoords(state, key) {
  if (!state) return { lat: null, lng: null }
  const st = state.toUpperCase().trim()
  const c = STATE_CENTROIDS[st]
  if (!c) return { lat: null, lng: null }
  const h = hash(key || st)
  const dlat = ((h & 0xffff) / 0xffff - 0.5) * 2.4
  const dlng = (((h >> 16) & 0xffff) / 0xffff - 0.5) * 2.4
  return { lat: +(c[0] + dlat).toFixed(6), lng: +(c[1] + dlng).toFixed(6) }
}