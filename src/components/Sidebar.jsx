...
export default function Sidebar({
  ...
  const clearFilters = () => {
    setFilters({ ...filters, search: '', services: new Set(), statuses: new Set() })
  }
  ...
  const sorted = useMemo(() => {
    ...
    return [...filteredSubs]
      .map(sub => {
        ...
        return {
          ...sub,
          _dist: turf.distance(from, turf.point([sub.lng, sub.lat]), { units: 'miles' }),
        }
      })
      .sort((a, b) => a._dist - b._dist)
  }, [filteredSubs, jobLocation])

  const hasFilters = filters.search || filters.services.size || filters.statuses.size
    ...
    <aside className="sidebar">
      <div className="sidebar-controls">
        <div className="sidebar-header sidebar-header-slim">
          <div className="sidebar-title">
            ...
            <div className="header-actions">
              {onAdd && <button className="icon-btn light" onClick={onAdd} title="Add subcontractor"><PlusIcon /></button>}
              {onImport && <button className="icon-btn light" onClick={onImport} title="Import CSV"><UploadIcon /></button>}
            </div>
          </div>
        ...
        <details className="filter-panel" open>
          <summary>Location and radius</summary>
          <div className="filter-panel-body">
            ...
            <div className="slider-row filter-radius-row">
              <input type="range" min="5" max="500" step="5" value={radius} onChange={event => setRadius(+event.target.value)} />
        ...
        <details className="filter-panel" open>
          <summary>Services and status</summary>
          <div className="filter-panel-body">
            <div className="filter-label">Services</div>
            <div className="chips service-chips">
            ...
            <div className="filter-label status-label">Status</div>
            <div className="chips">
              {STATUSES.map(status => (
                <button
                  type="button"
      ...
      <div className="results-header">
        <div>
          ...
          {jobLocation && <span> · sorted by distance</span>}
        </div>
        {hasFilters && <button type="button" className="clear-filter-btn" onClick={clearFilters}>Clear filters</button>}
      </div>
        ...
        {sorted.map(sub => (
          ...
          >
            <div className="sub-name-row">
              ...
              {sub._dist != null && sub._dist !== Infinity && <span className="sub-dist">{sub._dist.toFixed(0)} mi</span>}
            </div>
            ...
            {sub.canonicalServices?.length > 0 && (
              <div className="sub-services">
                {sub.canonicalServices.map(service => <span key={service} className="mini-chip">{service}</span>)}
              </div>
            )}
          </div>
        ))}
        {!sorted.length && <div className="empty-results">No subcontractors match the selected filters.</div>}
      </div>
    </aside>
  )
}
